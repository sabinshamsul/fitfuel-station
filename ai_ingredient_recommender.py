...
<<<<<<< HEAD
        ...
        'glycerine': Ingredient('Glycerine (Food Grade)', 6, 12, 4, 0, 0, 0, 'A humectant that keeps the bar moist and soft.', image_file='glycerine.png', is_binder=True),
        'maltodextrin': Ingredient('Maltodextrin', 0, 10, 5, 0, 95, 0, 'A very fast-digesting carb for rapid energy replenishment.', image_file='maltodextrin.png', is_carb=True),
        'pumpkin_seed_flour': Ingredient('Pumpkin Seed Flour', 0, 12, 6, 33, 14, 49, 'A whole-food carb source with protein and micronutrients.', image_file='pumpkin_seed.png', is_carb=True),
        'brown_rice_flour': Ingredient('Brown Rice Flour', 0, 10, 7, 7, 77, 3, 'A complex carbohydrate for sustained energy release.', image_file='brown_rice_flour.png', is_carb=True),
        'leucine': Ingredient('Leucine', 0, 4, 8, 100, 0, 0, 'A key amino acid that triggers muscle protein synthesis.', image_file='leucine.png', is_protein=True),
        'guarana_extract': Ingredient('Guarana Extract', 0, 2, 9, 0, 0, 0, 'A natural source of caffeine for energy and focus.', image_file='guarana.png'),
        'salt': Ingredient('Salt', 0.2, 0.8, 10, 0, 0, 0, 'Replenishes electrolytes lost during exercise.', image_file='salt.png'),
        'cocoa_powder': Ingredient('Cocoa Powder', 0, 3, 11, 20, 58, 14, 'Adds rich chocolate flavour and antioxidants.', image_file='cocoa.png', is_flavour=True),
...
<<<<<<< HEAD
        ...
        if user_selected:
            ...
            for ing_id in user_selected:
                if ing_id in self.INGREDIENTS and self.INGREDIENTS[ing_id].is_flavour:
                    # Map the selected ingredient ID back to a flavour name
                    if ing_id == 'cocoa_powder':
                        flavour = 'chocolate'
                        break
                    if ing_id == 'instant_coffee':
                        flavour = 'coffee'
                        break
                    if ing_id == 'matcha_powder':
                        flavour = 'matcha'
                        break
        ...
        if mode == 'pre':
            if carb_mode == 'maltodextrin':
                ...
                return {
                    ...
                    'rice_syrup': 16.0,
                    'glycerine': 9.0,
                    'pumpkin_seed_flour': 16.0,
                    'brown_rice_flour': 10.0,
                    'leucine': 1.5,
                    'salt': 0.5,
                    'guarana_extract': 0.0
                }
            ...
            if carb_mode == 'maltodextrin':
                ...
                return {
                    ...
                    'rice_syrup': 15.0,
                    'glycerine': 9.0,
                    'pumpkin_seed_flour': 14.0,
                    'brown_rice_flour': 11.0,
                    'leucine': 2.5,
                    'salt': 0.5,
                    'guarana_extract': 0.0
=======
...
<<<<<<< HEAD
            ...
            if carb_mode == 'maltodextrin':
                ...
                safe_adjust('glycerine', actual_increase * 0.1, is_increase=False)
            else:
...
<<<<<<< HEAD
    def _add_flavour_system(self, formulation: Dict[str, float], flavour: str, carb_mode: str) -> Dict[str, float]:
        ...
        # Compensate for the added flavour by reducing a low-priority carb
        if flavour_amount > 0:
            compensation_target = 'maltodextrin' if carb_mode == 'maltodextrin' else 'brown_rice_flour'
            if compensation_target in formulation:
                current = formulation.get(compensation_target, 0)
...
<<<<<<< HEAD
        ...
        for ing in user_selected:
            if ing in self.INGREDIENTS and self.INGREDIENTS[ing].is_flavour:
...
<<<<<<< HEAD
            ...
            is_flavour = False
            if ingredient in self.INGREDIENTS:
                is_flavour = self.INGREDIENTS[ingredient].is_flavour
            
            keep = False
            if ingredient in user_selected:
                keep = True
            elif ingredient in essentials:
                keep = True
            elif is_flavour and not user_picked_flavours:
                # Keep default flavour (e.g. vanilla) if user didn't manually pick a different flavour
                keep = True
...
<<<<<<< HEAD
        ...
        if carb_mode == 'maltodextrin':
            ...
            rice_flour = formulation.get('brown_rice_flour', 0)
            
            if pumpkin > 0.5:
                errors.append(
                    f"Maltodextrin mode contains {pumpkin:.2f}% pumpkin seed flour (incompatible)"
                )
            if rice_flour > 0.5:
                errors.append(
                    f"Maltodextrin mode contains {rice_flour:.2f}% brown rice flour (incompatible)"
                )
        else:
            maltodextrin = formulation.get('maltodextrin', 0)
...
<<<<<<< HEAD
    def _emergency_fallback(self, mode: str, carb_mode: str, flavour: str, user_selected: List[str] = None) -> Dict[str, float]:
        ...
        # Determine if vanilla should be included
        include_vanilla = True
        if user_selected:
            # Check if user manually selected any flavour
            user_picked_flavour = False
            for ing in user_selected:
                if ing in self.INGREDIENTS and self.INGREDIENTS[ing].is_flavour:
                    user_picked_flavour = True
                    break
            
            # If user picked flavours but NOT vanilla, exclude vanilla
            if user_picked_flavour and 'vanilla' not in user_selected:
                include_vanilla = False
                self.logger.info("Fallback: Excluding vanilla (user selected other flavours)")
        
        vanilla_amt = 0.2 if include_vanilla else 0.0
        ...
        if carb_mode == 'maltodextrin':
            ...
            base = {
                ...
                'rice_syrup': 16.0,
                'glycerine': 9.0,
                'pumpkin_seed_flour': 18.0,
                'brown_rice_flour': 12.0,
                'leucine': 2.0,
                'salt': 0.5,
                'guarana_extract': 0.0
=======
...
<<<<<<< HEAD
            ...
            if vanilla_amt > 0: base['vanilla'] = vanilla_amt
            base['maltodextrin' if carb_mode == 'maltodextrin' else 'brown_rice_flour'] -= (1.5 + vanilla_amt)
        elif flavour == 'matcha':
            base['matcha_powder'] = 0.5
            if vanilla_amt > 0: base['vanilla'] = vanilla_amt
            base['maltodextrin' if carb_mode == 'maltodextrin' else 'brown_rice_flour'] -= (0.5 + vanilla_amt)
        elif flavour == 'none':
            if vanilla_amt > 0:
